...
router.post("/questions", ensureAuthenticated, function(req, res, next) {
  var question = {
  ...
  Question.create(question, function(err) {
    ...
    req.flash("info", "Question made!");
    res.redirect("/questions");
  });
});

router.post("/questions/:question", ensureAuthenticated, function(req, res, next) {
  //To improve: Here we should test if the question already exists
  var response = {
  		"data": req.body.response,
  		"author": req.user._id,
  		"question": req.params.question
  };
  Response.create(response, function(err) {
    if (err) {
      next(err);
      return;
    }
    req.flash("info", "Response made!");
